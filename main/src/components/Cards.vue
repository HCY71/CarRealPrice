<template>
  <div id="cards">
        <div class="card" v-for="post in $store.state.posts.post" :key="post">
          <div class="header">{{post.author}} | {{post.menu.location}} | {{post.menu.car.brand}}
            <div class="date">{{post.postTime.date}}</div>
          </div>
          <div class="title">{{post.menu.car.year}} {{post.menu.car.brand}} {{post.menu.car.model}} {{post.menu.car.type}}</div>
...
#cards {
  ...
  .card:hover{
    box-shadow: -2px 3px 10px 1px rgba(black,.15);
